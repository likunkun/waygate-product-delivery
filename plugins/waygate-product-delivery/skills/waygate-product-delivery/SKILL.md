...
默认休眠。说 `启动交付` 激活当前项目的产品交付模式，并立即进入 `authorization_pending` 等待模式选择；说 `启动交付，多 Agent 模式` 显式授权当前 delivery 在结构化 review gate 自动启动 2–3 个独立 subagents；只有在真实 subagents 不可用时，才使用 `启动交付，允许降级评审` 显式允许 role_simulation 弱证据；说 `停止交付` 或使用 `stop` 退出干预。底层命令仍保留 `start` / `stop`。
