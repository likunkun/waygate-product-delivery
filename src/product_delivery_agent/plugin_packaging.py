...
def _write_templates(templates_dir: Path) -> None:
    templates = {
        ...
        "multi-agent-test-implementation-review.md": (
            ...
            "同轮全部评审者必须基于同一输入快照完成独立评审、交叉质疑和修订；"
            "全部评审者返回前不得修改评审对象。汇总者必须先去重并冻结完整问题清单，"
            "再批量修复全部已接受问题，最后执行一次统一复验。评审者失败时必须重试、替换"
            "或记录 blocked_with_reason，不得提前进入修复。\n"
        ),
...
def _skill_markdown() -> str:
    return (
        ...
        "每次新交付必须生成独立 `delivery_id`，并将 `multi_agent_policy` 同时绑定当前 "
        "`delivery_id` 与 `feature_slug`。收到启动请求时，先调用只读 "
        "`inspect_startup_request(feature_slug=...)`；已关闭旧 feature 启动新 feature 时必须归档旧 state、"
        "创建新 delivery 并重新选择评审模式，未终态的其他 feature 不得被静默覆盖。\n\n"
        "模型选择完全由用户和 Codex 宿主管理。Product Delivery 不选择、不记录、不验证、"
        "也不声称当前线程或 subagent 使用了任何特定模型。\n\n"
        "恢复 v1.0.19–v1.0.21 的 active state 时，如果存在旧 `execution_model_policy`，"
        "必须调用公共 `retire_model_execution_policy()` 归档旧 policy、清理模型 blocker 并保留当前 "
        "delivery、feature、原型、review 和确认；不得重新 start 当前 delivery，也不得手改 state。"
        "旧模型 API 只保留一版明确退役错误。\n\n"
        ...
        "再进行产品/场景评审。评审通过后调用 `prepare_product_baseline_confirmation()` 和 "
        "`confirm_product_baseline()`，先确认需求范围和 UI 原型或非 UI 行为契约。"
        "产品基线确认前不得生成详细测试用例、planned E2E 或 coverage audit。"
        "基线确认后才生成测试覆盖计划，完成 test/test_coverage review 后调用 "
        "`prepare_test_coverage_confirmation()` 和 `confirm_test_coverage_plan()`。"
        "两次正式确认完成前禁止实现。\n\n"
        "禁止实现，直到以下门禁全部满足：\n\n"
        ...
        "禁止实现的条件：未完成 `product_baseline` 和 `test_coverage_plan` 两次确认、"
        ...
        "通过前禁止声明完成。\n\n"
        "UI 原型是 `product_baseline` 的一部分。原型、prototype contract 或截图集合只有在"
        "产品/场景评审通过后，才通过 `prepare_product_baseline_confirmation()` 生成当前 nonce；"
        "用户使用 `confirm_product_baseline()` 一次确认需求范围和最终原型。"
        "截图、Playwright evidence 和 static review 只能作为辅助证据，不能替代该确认；"
        "裸 `继续` 不能替代当前 nonce。只有用户明确提出需求或原型修改，才允许调用 "
        "`record_user_requested_change()` 重开基线；内部评审不得自行改原型或制造重复确认。\n\n"
        ...
        "`new_surface_in_existing_product` 和 `greenfield_ui` 必须记录有意义的 "
        ...
        "第二次 `test_coverage_plan`，确认 planned E2E 和测试覆盖计划。"
        "需求或原型变化会同时使两次确认 stale；仅测试覆盖语义变化只使第二次确认 stale；"
        "测试 ID 拆分、内部断言增强和测试实现修复不得使产品基线 stale。"
        "legacy `confirm_ui_prototype()`、独立 Open Spec/E2E confirmation 和 combined confirmation "
        "只用于迁移兼容，现代 delivery 不再编排这些入口。"
        ...
        "`wait_for_user` 只允许用于真实用户输入点：当前 `product_baseline` 确认、"
        "`test_coverage_plan` 确认、评审模式选择、必要需求澄清、"
        "用户主动暂停或停止。"
        ...
        "V1.0.24 起，内部 `delivery_goal` 只表示 TASK 计划，不得冒充 Codex Host Goal。"
        "第二次 `test_coverage_plan` 确认同时授权当前 delivery 创建和推进真实 Host Goal，"
        "runtime 使用独立 `host_goal_binding` 绑定 `delivery_id`、`feature_slug`、launch package、"
        "objective hash、binding generation 和宿主返回的稳定标识。\n\n"
        "handoff 后必须按顺序执行：先调用 `prepare_host_goal_activation()`，再调用宿主 `get_goal`；"
        "确认不存在 Goal 后才调用 `create_goal`；创建后再次调用 `get_goal`。每次结果都必须通过 "
        "`record_host_goal_observation(checkpoint_id, observation)` 落盘。其他未完成 Goal、objective 不匹配、"
        "checkpoint 重放、stale observation 或 Goal 工具不可用都必须 fail closed。Goal 工具不可用时"
        "记录 unavailable observation 并阻塞，不能声称自动续跑已经生效。\n\n"
        "每个 post-handoff turn 开始，以及 TASK/stage、review、closure、final、stop 和 Goal 状态更新前，"
        "调用 `prepare_host_goal_reconciliation(operation, target_gate, host_turn_id=...)`，再使用 "
        "`get_goal` 或该 checkpoint 指定的 `update_goal`，最后调用 `record_host_goal_observation()`。"
        "同一 stage 内低层文件写入不需要重复查询，但任何 canonical gate 写入都必须消费一条新鲜、"
        "目标 gate 匹配的一次性 observation。\n\n"
        "`wait_for_user` 必须生成并复用稳定 `decision_id`、prompt hash 和 blocker fingerprint；"
        "同一问题只展示一次，自动 Goal turn 不得重复提问，也不得修改产品、测试、证据或 closure。"
        "用户回复必须匹配当前 `decision_id`，并通过 `user_resume` 再次观察 Host Goal active 后才能继续。"
        "只有三个不同 `host_turn_id` 连续观察到相同 blocker，才允许调用 `update_goal(status=blocked)`。\n\n"
        ...
        "authorization、generation、nonce、objective 和 journal hash chain，归档旧 checkpoint，追加 "
        "`host_goal_checkpoint_superseded` transition，并以相同 binding identity 重新签发 "
        "`inspect_before_activation` checkpoint。不得手改 state、重新 start delivery 或重放旧 checkpoint。\n\n"
        "V1.0.26 起，每个 delivery 在启动时把宿主 `CODEX_THREAD_ID` 冻结为 `host_goal_owner` 的 "
        "coordinator thread。Host Goal activation、observation、reconciliation、post-handoff canonical "
        "write 和 completion 必须同时匹配当前 `CODEX_THREAD_ID`、owner thread、binding owner 和宿主 "
        "Goal 的 `threadId`；缺失或不匹配全部 fail closed。spawned review subagent 只能产出结构化评审，"
        "不得激活、恢复、接管或完成 Host Goal。\n\n"
        "v1.0.25 及更早的非终态 state 缺少 owner metadata 时迁移为 `legacy_unverified`，不得从旧 "
        "binding 推断 coordinator。必须在没有 active/blocked Goal 的新用户可见顶层线程中使用固定授权语 "
        "`恢复交付主线程，接管当前 Host Goal`，依次调用 `prepare_host_goal_owner_claim()`、宿主 `get_goal`、"
        "`record_host_goal_owner_claim_observation()`。只有 `get_goal` 返回 missing 或 complete 才允许 transfer；"
        "旧 binding 和未消费 checkpoint 完整归档为 `orphaned_unreachable`，journal 追加 "
        "`host_goal_owner_transferred`，然后使用新 generation、nonce 和精确 objective 重新执行 "
        "`get_goal -> create_goal -> get_goal`。active/blocked foreign Goal 禁止覆盖。owner-claim checkpoint "
        "若因合法 transition 过期，必须调用 `recover_stale_host_goal_owner_claim(checkpoint_id)` 归档旧 claim、"
        "追加 `host_goal_owner_claim_superseded` 并签发新 claim；不得手删 `pending_claim`。\n\n"
        "active Waygate delivery 只能使用当前安装的 `waygate-product-delivery` runtime。禁止调用旧 "
        "`product-delivery-agent@1.0.8` runtime 写入同一 delivery；旧 runtime 产生的历史 evidence 可以保留，"
        "但后续 canonical transition 必须由当前 Waygate runtime 完成。\n\n"
        "V1.0.27 起，`start()` 必须写入当前 Waygate 的 `runtime_provenance`（插件名、版本和 runtime "
        "package hash）、非空 delivery ID、owner metadata，以及 hash-linked `delivery_activated` journal event。"
        "`status()` 只能将 active state 报告为 `verified_waygate`、`legacy_unverified` 或 "
        "`invalid_runtime`。缺少或不匹配 receipt、policy、owner 或 journal 的 active state 不得推进任何门禁。"
        "必须调用 `recover_legacy_active_delivery()` 归档原始 state 后创建新 delivery；不得手改 state，"
        "也不得复用旧 confirmation、review 或 implementation authorization。\n\n"
        "V1.0.28 起，已确认 UI 的 `implementation_baseline` 是实现阶段的权威产品基线，"
        "不是重新设计提示。它必须只读绑定 prototype design bundle、prototype contract、产品 HTML、"
        "设计系统、纯净截图、surface/state/viewport、region、interaction 及其 hash。"
        "用户可见 TASK 必须声明 `ui_impact` 并通过 `prototype_bindings` 绑定当前任务涉及的精确原型切片；"
        "非 UI TASK 必须声明 `ui_impact=none` 及理由。handoff 和当前 TASK 提示只注入当前绑定切片，"
        "明确要求保持 route、区域层级、控件顺序、关键状态和交互结果一致；允许调整内部代码架构，"
        "禁止自行增删、移动、合并或重设计可见 UI。无法按原型实现时必须停止 TASK 并发起 CR，"
        "不得静默降级。\n\n"
        "每个 UI TASK 完成前必须调用 `record_task_prototype_conformance()`，记录生产截图、"
        "semantic snapshot、结构与交互观察、geometry、computed-style fingerprint 和像素差异。"
        "route、region 存在性、父子层级、显示顺序、可访问角色/名称、必要控件及绑定 interaction "
        "零容忍；关键区域像素差异默认不超过 2%，全页面不超过 5%，单像素阈值为 0.2；"
        "geometry 偏差默认不超过 4 CSS px 或 viewport 的 1%，取较大值。证据缺失是 `failed`，"
        "渲染环境不稳定是 `inconclusive`，两者都阻止 TASK 完成。检查失败必须修复生产实现，"
        "不得修改冻结原型、mask 或阈值。确需改变可见设计时必须走 CR 并重开产品基线。\n\n"
        "产品域、视觉阈值、mask 或 TASK binding 变化必须让相关 TASK、conformance evidence 和实现授权 stale；"
        "仅 `review_annotation_set` 变化不影响 implementation baseline。新 UI delivery 强制执行上述规则；"
        "已确认 product baseline 的 active UI delivery 保持 grandfathered，除非原型被重开，"
        "重开后必须升级到 V1.0.28 门禁。最终 closure 仍需 full-stack Browser E2E、"
        "全量 production conformance 和独立 `ui_conformance` review。\n\n"
        "V1.0.29 起，权威 TASK 队列由 runtime 从 planned E2E obligations 按 Journey 派生。"
        "默认一个 Journey 一个 TASK；超过 action/item/surface/happy-path 阈值必须再拆。"
        "不允许独立脚手架 TASK。第一个 Journey TASK 只吸收最小壳，后续 Journey 复用。"
        "UI obligation 必须声明 `surface_ids`；TASK bindings 只等于其 obligation 并集。"
        "Agent 只能细化标题、描述、验证说明和更窄绑定，不能合并、删除或转移 E2E。"
        "派生后回写 coverage `task` 列，并把 `task_queue_hash` 纳入 test_coverage 评审和确认。"
        "当前 TASK 必须先通过本切片 `record_task_executed_evidence()` 和原型一致性才能完成。"
        "全部切片通过后，`record_executed_browser_evidence()` 只接受切片证据并集加回归。"
        "新 delivery 强制生效；已确认 `test_coverage_plan` 的活跃 delivery 保持 grandfathered，"
        "直到测试覆盖被重开。\n\n"
        "V1.0.30 起，执行期 `test_implementation` 和 `ui_conformance` multi-agent review "
        "必须实行批次屏障：同轮 2–3 个评审者基于同一输入快照独立完成 positions、cross challenges "
        "和 revisions，全部评审者返回前不得修改评审对象；汇总者先去重并冻结完整问题清单，再批量修复"
        "本轮全部已接受问题，完成后只做一次统一复验。评审者失败必须重试、替换或记录 "
        "`blocked_with_reason`，不得收到一个问题就修改并重启评审。UI TASK 像素比较仍以关键区域 2% "
        "和完整页面 5% 为自动通过目标。首次纯像素失败后必须使用 `superpowers:systematic-debugging` "
        "连续完成两轮真实修复复验；相同证据、inconclusive 环境或非像素失败不计数。两轮仍失败才创建"
        ...
        "该模式还必须记录 2–3 个唯一的 `reviewer_agent_ids` 和真实 `reviewer_spawn_source`；"
        "模型名称不是 review evidence。"
...
def _hooks_readme() -> str:
    return (
        ...
        "This plugin does not provide a timed continuation hook and does not send "
        "a synthetic `继续` after 20 seconds. Cross-turn continuation is coordinated "
        "through a verified Codex Host Goal binding. Human-decision gates remain "
        "paused until a matching user response is recorded. The Host Goal owner is "
        "the top-level delivery coordinator captured from `CODEX_THREAD_ID`; hooks "
