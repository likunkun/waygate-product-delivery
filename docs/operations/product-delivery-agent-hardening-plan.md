...
`sample-target-project` V1.0.3 复测修正了一个判断：第一次本地 HTML 原型生成后，agent 确实让用户确认了；真实失败是用户指出“缺少人员与模板的编辑部分”后，第二版 prototype 没有重新让用户确认，而是把后续裸 `继续` 当成确认继续实现。
...
`sample-target-project` V2.5 复测显示：V1.0.2 已能让 agent 生成 Open Spec、本地 HTML 原型、截图、E2E 和 subagent review，但这些产物仍可能停留在聊天记录、Open Spec 摘要或普通文件中，没有成为不可跳过的 Product Delivery 状态机门禁。
...
V1.0.3 不改变此前结论：`sample-target-project` 的需求范围控制问题仍按用户反馈视为误报，本轮修复不以 changed-files scope audit 作为 P0。

## 结论

`sample-target-project` 试运行证明当前插件能生成 Open Spec、HTML prototype、截图证据、测试脚本和 closure artifact，但它仍然没有可靠接管交付流程。核心问题不是产物完全缺失，而是流程可以在关键确认点缺失、证据不合格或状态自称通过的情况下继续往前走。

重要修订：此前关于 `sample-target-project` 需求边界失控的判断已撤销，按用户反馈认定为误报。后续改造不再把该项作为失败根因，也不围绕该误报设计 P0 修复项。文档和模板中保留的 `scope` 只表示“版本边界与场景映射”，不是重新认定需求范围控制失败。
...
下一次 `sample-target-project` 类试运行只有满足以下条件才算合格：
...
`sample-target-project` V2.7 复测显示：流程已经能做到原型二次确认、真实 spawned subagent review、精确实现授权、完整 TASK 队列、executed browser evidence 和 validator 后 local closure。但最后仍暴露出一个 closure authority 问题：目标项目把 `scripts/verify/validate-closure-artifact.py` 改成 feature-specific rules map，并让 `status=PASS_WITH_NOTES` 的自定义 `formal-closure.json` 通过。
...
`sample-target-project` V2.8 复测显示：原型确认、多 agent review、实现授权、TDD、browser E2E、readonly smoke 和 redaction scan 都已明显改善；剩余失败集中在 Product Delivery 本地权威链路：
...
## V1.0.9 Multi-Agent Test Coverage Review Gate

`sample-target-project` V2.8 后续事故结论收敛为：问题不是需求范围没包含，而是测试覆盖和测试实现没有被有效审查。R5 原型、测试审计和 scenario review 已经写到二级工作台、三级详情、人员维护、模板、Agent 规则、绑定、供应商、白名单、告警忽略等内容，但后续 TC 标题覆盖了这些场景，测试动作没有真正逐项点击和断言真实功能面板。

V1.0.9 新增硬规则：

- 泛化 `multi_agent_test_review` 不能替代测试覆盖范围评审。
- 实现授权前必须通过 `multi_agent_test_coverage_review`。
- `test_coverage` review 的对象是测试用例覆盖范围，不是原型、需求或普通 scenario review。
- `test_coverage` review 必须审查 `US/J/SC/AC/TASK/TC` 映射。
- 集合型场景必须展开 item-level coverage，例如二级工作台 tab、三级详情入口、人员维护、模板、Agent 规则、绑定、供应商、白名单、告警忽略。
- planned E2E obligations 必须写入 `coverage_items`、`action_assertions` 和 `false_positive_guards`。
- 每个 `coverage_item` 必须有具体 `action_entry`、`expected_real_surface`、`assertion_target` 和 `semantic_depth`。
- `marker_only`、`function_name_only`、`static_panel_only`、`first_button_only` 必须被判定为 false-positive risk。
- 实现和 E2E 运行后、formal closure 前必须通过 `multi_agent_test_implementation_review`。
- `test_implementation` review 的对象是真实测试代码、Playwright/browser 脚本、执行日志、截图和 trace。
- `test_implementation` review 必须确认每个 TC 真的点击入口、进入真实功能面板、断言真实控件或行为。
- 发现 coverage gap 后，必须先补 RED test，让当前浅实现失败，再允许继续修 UI 或 E2E。

验收补充：

- UI 项目缺 `test_coverage` multi-agent review 时，pre-handoff 必须失败。
- UI 项目缺 `test_implementation` multi-agent review 时，pre-closure 必须失败。
- TC 标题声称覆盖集合型场景，但 action assertions 少于 required items 时必须失败。
- 只检查 DOM marker、函数名、静态说明面板或第一个按钮时必须失败。
- implementation review 只引用计划/原型，不引用真实测试代码和执行证据时必须失败。
